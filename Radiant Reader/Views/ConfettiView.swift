...
// Borrowed and modified for demo purposes from https://github.com/NSHipster/ConfettiView
/// A view that emits confetti.
public final class ConfettiView: UIView {

    private static let kAnimationLayerKey = "com.radiantReader.animationLayer"
     ...
     */
    public func emit(for duration: TimeInterval = 3.0, with contents: [Content])
    {
        let layer = Layer()
    ...
    private final class Layer: CAEmitterLayer {
        func configure(with contents: [Content]) {
            emitterCells = contents.map { content in
                ...
                cell.scale = 0.75 - cell.scaleRange
...
extension ConfettiView: CAAnimationDelegate {
    public func animationDidStop(_ animation: CAAnimation, finished flag: Bool) {
        if let layer = animation.value(forKey: Self.kAnimationLayerKey) as? CALayer {
